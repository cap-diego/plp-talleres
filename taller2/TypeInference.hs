...
-- intersect l1 l2 = filter (\e -> elem e l2) l1
intersect c1 c2 = filter (\t -> elem t c2) c1
...
infer' (AppExp u v) n =
    case infer' u n of
      	OK (n', (c', u' , t') ) ->
			case infer' v n' of
				OK (n'', (c'', v' , t'') ) ->
					case mgu [ (t', TFun t'' (TVar (n'' + 1) ))] ++ (intersect (c')(c'')) of
						UOK subst -> OK(n'' + 1,
										(
											subst <.> joinC [c', c''],
											subst <.> (AppExp u' v'),
...
-- para s en la interseccion
-- verificar que eval (s, c') == eval(s, c'')
